  ...
  (loop [state (status (new-game))]
    (let [location (get-in state [:adventurer :location])
         ...
         (do
           ...
           (println "-inventory: check the inventory (to use, type in i or inventory)")
           ...
           (println "-cookfish: cook the fish to make it the baked fish")
           (println "-cookegg: cook the egg to make it the baked egg")
           (println "-eat: eat something (example: to eat :raw-egg, just type in raw-egg)")
           (println "-unlock: unlock the chest to see what's inside (the inside item is essential to escape (win the game))")
           (println "-escape: escape from the grave and win the game (you will need a rope and be at the east-most (right-most) room)")
